    ...
    var body: some View {
        NativeSideNavigation(onNavigate: handleNavigation) {
            ZStack(alignment: .topLeading) {
                ...
                // [media-player-content-patch]
                // Media Player Overlay
                if let mediaPlayerData = uiState.mediaPlayerData {
                    NativeMediaPlayerView(data: mediaPlayerData)
                        .position(
                            x: mediaPlayerData.frame.x + mediaPlayerData.frame.width / 2,
                            y: mediaPlayerData.frame.y + mediaPlayerData.frame.height / 2
                        )
                        .frame(
                            width: mediaPlayerData.frame.width,
                            height: mediaPlayerData.frame.height
                        )
                }
            }
        }
